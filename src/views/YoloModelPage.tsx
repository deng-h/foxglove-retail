...
export default function YoloModelPage(): ReactElement {
  ...
  const handleFileChange = async (event: ChangeEvent<HTMLInputElement>): Promise<void> => {
    ...
    if (selectedFiles && selectedFiles.length > 0) {
      // YOLO模型支持的文件格式
      const allowedExtensions = ['.pt', '.pth', '.onnx', '.engine', '.tflite', '.pb'];
      ...
      if (invalidFiles.length > 0 || oversizedFiles.length > 0) {
        ...
        if (oversizedFiles.length > 0) {
          if (message) message += '；';
          message += `以下文件超过500MB，已跳过：${oversizedFiles.join(', ')}`;
        }
  ...
  const handleConfirmDelete = async () => {
    ...
    try {
      ...
      setSnackbarMessage('YOLO模型删除成功');
      setSnackbarSeverity('success');
      setSnackbarOpen(true);
      await loadFiles();
    } catch (error) {
      console.error('Failed to delete files:', error);
      setSnackbarMessage('YOLO模型删除失败');
      setSnackbarSeverity('error');
      setSnackbarOpen(true);
    } finally {
      setLoading(false);
  ...
  return (
    <Box sx={{ fontFamily: 'sans-serif', p: 3 }}>
      <Typography variant="h4" gutterBottom>YOLO模型文件管理</Typography>
      ...
      <input
        ...
        style={{ display: 'none' }}
        multiple
        accept=".pt,.pth,.onnx,.engine,.tflite,.pb"
      />

      <Paper elevation={2} sx={{ p: 2, borderRadius: 2 }}>
        <Typography variant="h6" gutterBottom>已上传的YOLO模型列表</Typography>
        <Typography variant="body2" color="text.secondary" sx={{ mb: 2 }}>
          支持格式: .pt, .pth, .onnx, .engine, .tflite, .pb (最大500MB)
        </Typography>
        <Divider sx={{ mb: 1 }} />
        {loading ? (
              ...
              <ListItem key={file.path} sx={{ borderBottom: '1px solid #eee', py: 1 }} disableGutters>
                ...
                <Box>
                  ...
                  <Typography variant="body2" color="text.secondary">
                    大小: {(file.size / 1024 / 1024).toFixed(2)} MB | 更新时间: {formatDateTime(file.updateAt)}
                  </Typography>
                </Box>
              </ListItem>
            ))}
          ...
          <Typography color="text.secondary">暂无YOLO模型，请点击上方按钮上传。</Typography>
        )}
      </Paper>
      <Snackbar
        open={snackbarOpen}
